...
The general setup is described in the [Contributor Setup Guide](../contributor/setup.md). This guide focuses on the dockerized setup process for PROMPT. The PROMPT setup process is straightforward and requires only a few environment variables to adjust.
...
- **Course-Specific Roles**  
  ...
  - **`<SemesterTag>-<CourseName>`** (Group and Role)
    ...
    Group: `Editor` (a subgroup of the course group). This role has limited user rights. For more details, refer to the [Contributor Guide](../contributor/setup.md).
...
For this option, refer to the [Contributor Setup Guide](../contributor/setup.md) on how to configure a standalone Keycloak instance and the `keycloakConfig.json`.
