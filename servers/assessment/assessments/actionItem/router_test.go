...
func (suite *ActionItemRouterTestSuite) TestCreateActionItemInvalidJSON() {
	phaseID := uuid.New()
	req, _ := http.NewRequest("POST", "/api/course_phase/"+phaseID.String()+"/action-item", bytes.NewBuffer([]byte("invalid json")))
...
func (suite *ActionItemRouterTestSuite) TestCreateActionItemValid() {
	...
	req, _ := http.NewRequest("POST", "/api/course_phase/"+phaseID.String()+"/action-item", bytes.NewBuffer(body))
	req.Header.Set("Content-Type", "application/json")
	resp := httptest.NewRecorder()
...
func (suite *ActionItemRouterTestSuite) TestGetActionItemsForStudentValid() {
	phaseID := uuid.MustParse("24461b6b-3c3a-4bc6-ba42-69eeb1514da9")
	partID := uuid.MustParse("ca42e447-60f9-4fe0-b297-2dae3f924fd7")
	req, _ := http.NewRequest("GET", "/api/course_phase/"+phaseID.String()+"/action-item/course-participation/"+partID.String(), nil)
	resp := httptest.NewRecorder()
...
func (suite *ActionItemRouterTestSuite) TestUpdateActionItemInvalidID() {
	phaseID := uuid.New()
	req, _ := http.NewRequest("PUT", "/api/course_phase/"+phaseID.String()+"/action-item/invalid-uuid", bytes.NewBuffer([]byte("{}")))
	req.Header.Set("Content-Type", "application/json")
	resp := httptest.NewRecorder()
...
func (suite *ActionItemRouterTestSuite) TestDeleteActionItemInvalidID() {
	phaseID := uuid.New()
	req, _ := http.NewRequest("DELETE", "/api/course_phase/"+phaseID.String()+"/action-item/invalid-uuid", nil)
	resp := httptest.NewRecorder()
...
func (suite *ActionItemRouterTestSuite) TestGetActionItemsForStudentInvalidIDs() {
	// invalid phase
	req1, _ := http.NewRequest("GET", "/api/course_phase/invalid-phase/action-item/course-participation/"+uuid.New().String(), nil)
	...
	phaseID := uuid.MustParse("24461b6b-3c3a-4bc6-ba42-69eeb1514da9")
	req2, _ := http.NewRequest("GET", "/api/course_phase/"+phaseID.String()+"/action-item/course-participation/invalid-uuid", nil)
