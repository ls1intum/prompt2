...
func (suite *ScoreLevelServiceTestSuite) TestGetScoreLevelByCourseParticipationID() {
	...
	assert.NoError(suite.T(), err)
	assert.Equal(suite.T(), db.ScoreLevelNovice, lvl, "Expected advanced level")
}
...
func (suite *ScoreLevelServiceTestSuite) TestGetStudentScore() {
	...
	assert.NoError(suite.T(), err)
	assert.Equal(suite.T(), db.ScoreLevelIntermediate, score.ScoreLevel, "Expected advanced level for student score")
