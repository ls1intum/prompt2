...
type GetCPPForCoursePhase struct {
	CoursePhaseID         uuid.UUID          `json:"coursePhaseID"`
	PassStatus            string             `json:"passStatus"`
	CourseParticipationID uuid.UUID          `json:"courseParticipationID"`
	RestrictedData        meta.MetaData      `json:"restrictedData"`
...
func GetCPPForCoursePhaseDTOFromDBModel(model db.GetSingleCoursePhaseParticipationIncludingPreviousRow) (GetCPPForCoursePhase, error) {
	restrictedData, err := meta.GetMetaDataDTOFromDBModel(model.RestrictedData)
	if err != nil {
		log.Error("failed to create CoursePhaseParticipation DTO from DB model")
		return GetCPPForCoursePhase{}, err
	}
